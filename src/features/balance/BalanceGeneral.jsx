...
const SectionCard = ({ title, icon, accentColor, items, total, totalLabel }) => {
 ...
 <span className={`text-sm font-medium ${item.tone || 'text-[var(--text-primary)]'}`}>{formatCurrency(item.value)}</span>
 </div>
 ))}
 </div>
...
const BalanceGeneral = ({ user }) => {
 ...
 <h2 className="nd-display text-[32px] font-medium tracking-tight text-[var(--text-display)]">Posición financiera operativa a partir de caja, CXC, CXP y saldos de apertura.</h2>
 ...
 <p className="mt-2 text-2xl font-medium text-[var(--text-secondary)]">{formatCurrency(balance.assets)}</p>
 </div>
 <span className="text-xl font-medium text-[var(--text-disabled)]">=</span>
 <div>
 <p className="nd-label text-[var(--text-disabled)]">Pasivos</p>
 <p className="mt-2 text-2xl font-medium text-[var(--accent)]">{formatCurrency(balance.liabilities)}</p>
 </div>
 <span className="text-xl font-medium text-[var(--text-disabled)]">+</span>
 <div>
 <p className="nd-label text-[var(--text-disabled)]">Patrimonio</p>
 <p className="mt-2 text-2xl font-medium text-[var(--success)]">{formatCurrency(balance.equity)}</p>
 </div>
 </div>
 </div>
 ...
 <h3 className="flex items-center gap-2 text-lg font-medium text-[var(--text-primary)]">
 ...
 <p className={`mt-3 nd-display text-[30px] font-medium ${balance.netWorkingCapital >= 0 ? 'text-[var(--success)]' : 'text-[var(--accent)]'}`}>
 ...
 <div>
 <p className="text-sm font-medium text-[var(--text-secondary)]">Notas del modelo</p>

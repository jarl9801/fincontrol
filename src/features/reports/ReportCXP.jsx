  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-[#d0d0e0]">Reporte de Cuentas por Pagar</h2>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-[#1a1a2e] rounded-xl p-6 shadow-sm border border-[#2a2a4a]">
          ...
          <p className="text-2xl font-bold text-[#f87171]">{formatCurrency(totalPayable)}</p>
          <p className="text-xs text-[#6868a0] mt-1">{payables.length} facturas</p>
        </div>
        ...
        <div className="bg-[#1a1a2e] rounded-xl p-6 shadow-sm border border-[#2a2a4a]">
          <div className="flex items-center justify-between mb-2">
            ...
            <TrendingDown className="text-[#34d399]" size={20} />
          </div>
            ...
            <tbody>
              {Object.values(agingAnalysis).map((item, idx) => (
                <tr key={idx} className="border-b border-[#2a2a4a]">
                  ...
                  <td className="py-3 text-sm font-medium text-right text-[#f87171]">{formatCurrency(item.amount)}</td>
                </tr>
              ))}
              <tr className="bg-[#13132a]">
                <td className="py-3 text-sm font-bold text-[#d0d0e0]">Total</td>
                <td className="py-3 text-sm font-bold text-[#d0d0e0] text-center">{payables.length}</td>
                <td className="py-3 text-sm font-bold text-right text-[#f87171]">{formatCurrency(totalPayable)}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-[#1a1a2e] rounded-xl shadow-sm border border-[#2a2a4a] overflow-hidden">
        <div className="px-6 py-4 border-b border-[#2a2a4a]">
          <h3 className="text-lg font-bold text-[#d0d0e0]">Detalle de Cuentas por Pagar</h3>
        </div>
              ...
              {payables.map(t => {
                ...
                return (
                  <tr key={t.id} className="hover:bg-[#13132a]">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-[#f87171] text-right">{formatCurrency(t.amount)}</td>
